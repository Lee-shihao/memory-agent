...
pub async fn search_memory_async(args: &HashMap<String, JsonValue>) -> String {
...
pub async fn search_skills_async(args: &HashMap<String, JsonValue>) -> String {
    let query = args.get("query").and_then(|v| v.as_str()).unwrap_or("");
    ...
    let new: Vec<_> = raw
        ...
        .collect();
    if new.is_empty() {
        return "No new skills found. Try a different query.".to_string();
    }
    for r in &new {
...
pub async fn pre_index_skills(config: &crate::config::Config) {
    let skills = crate::skills::cached_skills();
    if skills.is_empty() {
        return;
    }
    ...
    router.index_skills(&skills).await.ok();
}
